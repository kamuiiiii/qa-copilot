...
本仓库是某 **派单 / 订单系统**（foodtruck，qa / uat 环境）的 QA 执行仓库。

测试**方法论**（工作流四阶段、AC 的 Gherkin 纪律、截图与 AC 一一绑定、性能规则、浏览器接入 / cookie 注入 / 跨站约定、报告模板……）**全部由 `qa-copilot` skill 提供**，不在本仓库重复。用户用自然语言描述测试目标即可触发该 skill，或显式 `/qa-copilot`。
...
```text
/plugin marketplace add <qa-copilot-plugin 仓库地址>
/plugin install qa-copilot@qa-tools
```

依赖 `browser-use` skill 驱动浏览器。
