...
本项目默认走 **独立 headless 浏览器 + cookie 注入**：从一份事先导出的 cookie 文件导入登录态，在全新的 headless 实例里跑测试。这样 **不占用用户正在使用的 Chrome、不需要物理屏幕、不需要手动开远程调试端口**。被测系统的登录态是 **纯 cookie**（如 dispatch-portal 的 `SessionId`，httpOnly+secure 会话级 cookie），所以复制 cookie 即可绕过 SSO，无需自动化登录流程。
